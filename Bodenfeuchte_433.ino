 ...
 *    dtostrf(floatVar, minStringWidthIncDecimalPoint, numVarsAfterDecimal, charBuf);   //#include<stdlib.h>
 *  
...
#define OW_ROMCODE_SIZE           8
   ...
   OneWire  ds(DALLAS_SENSOR_PIN);     // on arduino port pin PB4 (a 4.7K resistor is necessary, between Vcc and DQ-Pin   1=GND 2=DQ 3=Vcc )
#endif
...
#define MAIN_PERIOD       162 //998   // 62 How long to count pulses, for a fixed frequency this should be 998 ms (for a total runtime of 1 sec with 1 MHz oscillator)
...
volatile float          tempDallasCelsius = 0.0;  
char                    buf[20];      
...
*/
  ...
  vcc_reading = readVcc(); 
  float controllerVCC =  vcc_reading / 1000.0; //--- from mV to Volt

  //--- Bodenfeuchte auslesen 
  bodenfeuchte = DoBodenFeuchteMeasurement();   
  
  //--- 18B20 get temperature   
  float temp = ReadSingleOneWireSensor(dallas); 
  
  #ifdef USE_WITH_NANO 
    #ifdef USE_SERIAL_OUTPUT_NANO
      Serial.print("Sensor-ID (Batterie): "); 
      Serial.println(SENSORID_BATTERIE); 
      Serial.print("Temperatur(18B20): "); 
      Serial.println(temp);             
    #endif 
  #endif 
  ...
  LaCrosse.bSensorId = SENSORID_BODENFEUCHTE;
  LaCrosse.t = temp;         //--- alias temperature;  
...
float ReadSingleOneWireSensor(OneWire ds)
{
  ...
  float dallas_temperatur = 123.123;
  ...
  //Serial.print ("+++ Temp in Grad [dallas_temperatur]: " );
  //Serial.println (dallas_temperatur );
